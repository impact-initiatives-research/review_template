...
raww <- readxl::read_excel("inputs/REACH_UKR2008_Dataset Raw non PII HERAIII_NOV2021.xlsx", guess = 10000)
cleann <- readxl::read_excel("inputs/REACH_UKR2008_Dataset clean HERAIII_NOV2021.xlsx", guess = 50000)
logg <- readxl::read_excel("inputs/REACH_UKR2008_Data cleaning log HERAIII_DEC2021.xlsx", guess = 50000,
                           sheet = "VCL")
dell <- readxl::read_excel("inputs/REACH_UKR2008_Data cleaning log HERAIII_DEC2021.xlsx", guess = 50000,
                           sheet = "GCL")

questions <- readxl::read_excel("inputs/REACH_UKR2008_Questionnaire HERAIII_NOV2021.xlsx")
choices <- readxl::read_excel("inputs/REACH_UKR2008_Questionnaire HERAIII_NOV2021.xlsx", sheet = 2)
...
other_log <- cleann %>% 
  select(any_of(text_oth), `_uuid`) %>% 
  pivot_longer(cols = -`_uuid`) %>% 
  filter(!is.na(value)) %>% 
...
Cleaning log ambigious for `r summary_cleanning %>% filter(is.na(check_clean), new.value != "NA") %>% nrow()` entries.  
```{r, warning = F}
summary_cleanning %>% filter(is.na(check_clean), 
                             new.value != "NA") %>% 
  group_by(question.name, new.value, value_clean) %>% tally(sort = T) %>% 
  knit_big_table()
